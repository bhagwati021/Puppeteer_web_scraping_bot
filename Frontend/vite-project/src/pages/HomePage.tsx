...
const HomePage: React.FC = () => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      const apiPromise = axios.get(
        `http://backend.bhagwatibashyal.site:5000/api/questions${activeCategory !== 'all' ? `?category=${activeCategory}` : ''}`
      );
  ...
  const fetchComments = async (questionId: string) => {
    ...
    try {
      const response = await axios.get(`http://backend.bhagwatibashyal.site:5000/api/comments/question/${questionId}`);
  ...
  const triggerScraping = async (questionId: string) => {
    ...
    try {
      const { data } = await axios.post(`http://backend.bhagwatibashyal.site:5000/api/scraping/scrape/${questionId}`);
  ...
  const addComment = async (questionId: string, text: string) => {
    ...
    try {
      await axios.post(`http://backend.bhagwatibashyal.site:5000/api/comments`, { userId: 'user123', questionId, text });
      fetchComments(questionId);
    } catch (error:unknown) {
      if (error instanceof Error) {
  ...
  const deleteQuestion = async (questionId: string) => {
    ...
    try {
      await axios.delete(`http://backend.bhagwatibashyal.site:5000/api/questions/${questionId}`);
